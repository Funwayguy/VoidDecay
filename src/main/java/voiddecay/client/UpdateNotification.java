...
public class UpdateNotification
{
	...
	{
		if(!VoidDecay.proxy.isClient() || hasChecked)
		{
			return;
		}
		...
		if(VoidDecay.VERSION == "VD_VER_" + "KEY")
		{
			event.player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED + "THIS COPY OF " + VoidDecay.NAME.toUpperCase() + " IS NOT FOR PUBLIC USE!"));
			return;
		}
		...
		try
		{
			...
			int verStat = compareVersions(VoidDecay.VERSION, version);
			
			if(verStat == -1)
			{
				event.player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED + "Update " + version + " of " + VoidDecay.NAME + " available!"));
			...
			{
				event.player.addChatMessage(new ChatComponentText(EnumChatFormatting.YELLOW + VoidDecay.NAME + " " + VoidDecay.VERSION + " is up to date"));
			} else if(verStat == 1)
			{
				event.player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED + VoidDecay.NAME + " " + VoidDecay.VERSION + " is a debug build"));
			} else if(verStat == -2)
			{
				event.player.addChatMessage(new ChatComponentText(EnumChatFormatting.RED + "An error has occured while checking " + VoidDecay.NAME + " version!"));
			}
